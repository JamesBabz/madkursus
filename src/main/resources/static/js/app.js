...
const categoryLabels = {
  BAKING:t("domain.bagning"), BREAD:t("domain.brod"), DAIRY:t("domain.mejeri"), EGG:t("domain.g2"), FISH:t("domain.fisk"), FROZEN:t("domain.frostvarer"),
  FRUIT:t("domain.frugt"), GRAIN_PASTA:t("domain.kornRisOgPasta"), HERB:t("domain.urter"), LEGUME:t("domain.baelgfrugter"), MEAT:t("domain.kod"),
  NUT_SEED:t("domain.nodderOgFro"), OIL_FAT:t("domain.olieOgFedt"), OTHER:t("domain.andet"), PRESERVED:t("domain.konserves"),
  SAUCE_CONDIMENT:t("domain.saucerOgTilbehor"), SPICE:t("domain.krydderier"), STOCK:t("domain.fondOgBouillon"),
  SWEETENER:t("domain.sodemidler"), VEGETABLE:t("domain.grontsager"), VINEGAR_ACID:t("domain.eddikeOgSyre"), DRY_GOODS:t("domain.torvarer")
};
const unitLabels = { GRAM: t("domain.gram"), MILLILITER: t("domain.milliliter"), PIECE: t("domain.styk") };
const categoryIcons = {
...
function apiError(response, body) {
  ...
  const error = new Error(localizeError(detail) || t("apiError.serverenSvaredeMedStatusStatus", {status: response.status}));
...
async function authenticate(event) {
  ...
  try {
    ...
    showMessage(authError, t("authenticate.loginMislykkedesMessage", {message: error.message}));
  } finally {
...
async function register(event) {
  ...
  if (data.get('password') !== data.get('passwordConfirmation')) {
    showMessage(authError, t("register.adgangskoderneErIkkeEns"));
    return;
  }
  ...
  try {
    ...
    registerForm.reset();
    showAuthMode('login');
    showMessage(authSuccess, t("register.kontoenErOprettetDuKanNuLogge"));
    document.querySelector('#login-username').value = data.get('username').trim();
    document.querySelector('#login-password').focus();
  } catch (error) {
    showMessage(authError, t("register.kontoenKunneIkkeOprettesMessage", {message: error.message}));
  } finally {
    button.disabled = false;
  }
}
...
async function searchTemplates(search) {
  ...
  } catch (error) { if (requestId === searchRequestId) showToast(t("searchTemplates.katalogetKunneIkkeHentesMessage", {message: error.message}), 'error'); }
}
...
async function addTemplate(id, button) {
  ...
  try {
    ...
    await loadProducts(); showToast(t("createProduct.nameErTilfojet", {name: created.name}));
...
function createProductCard(product) {
  ...
  card.setAttribute('aria-label', t("shoppingRow.redigerName", {name: product.name}));
...
async function updateProduct(event) {
  ...
  try {
    ...
    closeProductEditor();
    await loadProducts();
    showToast(t("saveCurrentMealPlan.nameErGemt", {name: updated.name}));
  } catch (error) { showMessage(editError, t("createProduct.produktetKunneIkkeGemmesMessage", {message: error.message})); }
  finally { button.disabled = false; }
}
...
async function loadProducts() {
  ...
  try {
    ...
    list.replaceChildren();
    showMessage(errorMessage, t("loadProducts.produkterneKunneIkkeHentesMessage", {message: error.message}));
  } finally {
    loading.hidden = true;
  }
}
...
function showView(view) {
  ...
  document.querySelector('#recipes-title').textContent = mealPlansActive ? t("showView.madplan") : t("showView.dineOpskrifter");
  document.querySelector('#recipes-title').previousElementSibling.textContent = mealPlansActive ? t("showView.dinPlanlaegning") : t("showView.opskrifter");
...
function displayUnit(unit) {
  return { GRAM: t("domain.g"), MILLILITER: t("domain.ml"), PIECE: t("domain.stk"), TEASPOON: t("domain.tsk"), TABLESPOON: t("domain.spsk"), DECILITER: t("domain.dl"), GRINDER_TURN: t("domain.omgange") }[unit] || unit;
}
...
function inventoryConversion(quantity, unit) {
  ...
  return `= ${converted} ${unit === 'GRAM' ? t("inventoryConversion.kg") : t("inventoryConversion.liter")}`;
}
...
function createInventoryCard(item) {
  ...
  card.setAttribute('aria-label', t("createInventoryCard.redigerLagerbeholdningForName", {name: item.product.name}));
  ...
  amount.textContent = item.product.inventoryTrackingMode === 'PRESENCE' ? t("requirementResultRow.paLager")
    : `${formatQuantity(item.quantity)} ${displayUnit(item.unit)}`;
  content.append(name, amount);
  if(item.product.inventoryTrackingMode==='PRESENCE'&&item.plannedUsageCount>0){const usage=document.createElement('button');usage.type='button';usage.className='reservation-link';usage.textContent=t("createInventoryCard.brugesIPlannedUsageCountPlanlagteValue2", {plannedUsageCount: item.plannedUsageCount, count: item.plannedUsageCount});usage.onclick=event=>{event.stopPropagation();openInventoryReservations(item);};content.append(usage);}
  if(item.product.inventoryTrackingMode==='QUANTITY'&&Number(item.reservedQuantity)>0){amount.textContent=t("createInventoryCard.value1Value2PaLager", {value1: formatQuantity(item.physicalQuantity), value2: displayUnit(item.unit)});const planned=document.createElement('button');planned.type='button';planned.className='reservation-link';planned.textContent=t("createInventoryCard.value1Value2Planlagt", {value1: formatQuantity(item.reservedQuantity), value2: displayUnit(item.unit)});planned.onclick=event=>{event.stopPropagation();openInventoryReservations(item);};const state=document.createElement('p');state.className=`inventory-availability${Number(item.plannedShortfall)>0?' shortfall':''}`;state.textContent=Number(item.plannedShortfall)>0?t("createInventoryCard.manglerValue1Value2TilPlanlagteRetter", {value1: formatQuantity(item.plannedShortfall), value2: displayUnit(item.unit)}):Number(item.availableQuantity)===0?t("createInventoryCard.intetLedigt"):t("createInventoryCard.value1Value2Ledig", {value1: formatQuantity(item.availableQuantity), value2: displayUnit(item.unit)});content.append(planned,state);}
...
function inventoryText(items) {
  return items.filter(item => item.product.inventoryTrackingMode !== 'UNTRACKED').map(item => {
    ...
    const amount = item.product.inventoryTrackingMode === 'PRESENCE' ? t("inventoryText.har")
      : `${formatQuantity(item.quantity)} ${item.unit === 'PIECE' ? t("renderShoppingTextResult.stk") : displayUnit(item.unit)}`;
    return `- ${name}: ${amount}`;
  }).join('\n');
}
async function copyInventory() {
  try { await navigator.clipboard.writeText(inventoryText(inventoryForCopy)); showToast(t("copyInventory.lagerKopieret")); }
  catch (error) { showToast(t("copyInventory.lageretKunneIkkeKopieresTilladAdgangTil"), 'error'); }
}
async function loadInventory() {
  ...
  try {
    ...
    document.querySelector('#inventory-list').replaceChildren();
    showToast(t("loadInventory.lageretKunneIkkeHentesMessage", {message: error.message}), 'error');
...
function renderShoppingTextResult(result) {
    ...
    row.textContent = item.error ? t("shoppingList.import.invalidLine", {line: item.line, value: item.text, error: localizeError(item.error)})
      : t(item.quantity == null
        ...
        : (item.newProduct ? "shoppingList.import.newQuantity" : "shoppingList.import.existingQuantity"), {
          ...
          unit: item.unit === 'PIECE' ? t("renderShoppingTextResult.stk") : displayUnit(item.unit)
        });
    return row;
  }));
  host.hidden = false;
  ...
  if (!result.valid) showMessage(document.querySelector('#shopping-text-error'), t("renderShoppingTextResult.retLinjerneMedFejlIngenVarerEr"));
}
async function submitShoppingText(confirm = false) {
  ...
  button.textContent = confirm ? t("submitShoppingText.tilfojer") : t("submitShoppingText.kontrollerer");
  showMessage(document.querySelector('#shopping-text-error'), '');
  try {
    ...
    if (result.imported) {
      ...
      await Promise.all([loadShoppingList(), loadProducts()]); showToast(t("submitShoppingText.listenErTilfojetTilIndkob"));
    } else {
    ...
    document.querySelector('#preview-shopping-text').textContent = t("submitShoppingText.visForslag");
    document.querySelector('#confirm-shopping-text').textContent = t("submitShoppingText.tilfojTilIndkob");
  }
}
...
async function searchInventoryCandidates(search) {
  ...
  } catch (error) { if (requestId === inventorySearchRequestId) showToast(t("searchRecipeTemplates.sogningenMislykkedesMessage", {message: error.message}), 'error'); }
}
...
async function submitInventoryCandidate(quantity) {
  ...
  try {
    ...
    showToast(t("purchaseShoppingItem.nameErTilfojetTilLageret", {name: name}));
...
async function saveInventory(event) {
  ...
  try {
    ...
    closeInventoryEditor(); await loadInventory(); showToast(t("saveInventory.lagerbeholdningenErGemt"));
...
async function deleteInventory() {
  ...
  try { await jsonRequest(`${INVENTORY_API}/${id}`, { method: 'DELETE' }); closeInventoryEditor(); await loadInventory(); showToast(t("deleteInventory.varenErFjernetFraLageret")); }
  catch (error) { showMessage(document.querySelector('#edit-inventory-error'), error.message); }
}
...
async function createProduct(event) {
  ...
  saveButton.textContent = t("createProduct.gemmer");
  ...
  try {
    ...
    setFormOpen(false);
    await loadProducts();
    showToast(t("createProduct.nameErTilfojet", {name: created.name}));
  } catch (error) {
    showMessage(errorMessage, t("createProduct.produktetKunneIkkeGemmesMessage", {message: error.message}));
  } finally {
    saveButton.disabled = false;
    saveButton.textContent = t("createProduct.gemProdukt");
  }
}

function requestProductDeletion() {
  const name = document.querySelector('#edit-product-name').value.trim();
  document.querySelector('#delete-product-question').textContent = t("requestProductDeletion.erDuSikkerPaAtDuVil", {name: name});
...
async function deleteProduct() {
  ...
  try {
    ...
    closeProductEditor(); await loadProducts(); showToast(t("deleteProduct.nameErFjernet", {name: name}));
  } catch (error) {
    const message = error.status === 409
      ? t("deleteProduct.produktetFindesStadigPaLagerFjernDet")
      : t("deleteProduct.produktetKunneIkkeFjernesMessage", {message: error.message});
...
function shoppingRow(item) {
  ...
  row.setAttribute('aria-label', item.purchased ? t("shoppingRow.nameKobt", {name: item.product.name}) : t("shoppingRow.markerNameSomKobt", {name: item.product.name}));
  ...
  quantity.textContent = item.product.inventoryTrackingMode === 'PRESENCE' ? t("shoppingRow.kob")
    : `${formatQuantity(item.quantity)} ${displayUnit(item.unit)}`;
  const edit = document.createElement('button'); edit.type = 'button'; edit.className = 'shopping-edit-button'; edit.textContent = '⋯';
  edit.setAttribute('aria-label', t("shoppingRow.redigerName", {name: item.product.name})); edit.hidden = item.purchased;
  ...
  if (!item.purchased) attachShoppingGestures(row, item);
    ...
    row.setAttribute('aria-label', t("shoppingRow.fortrydKobAfName", {name: item.product.name}));
...
async function loadShoppingList() {
  ...
  } catch (error) { showToast(t("loadShoppingList.indkobslistenKunneIkkeHentesMessage", {message: error.message}), 'error'); }
  finally { loadingElement.hidden = true; }
}
...
async function purchaseShoppingItem(item, quantity = undefined) {
  ...
  try {
    ...
    showToast(t("purchaseShoppingItem.nameErTilfojetTilLageret", {name: item.product.name}), 'success', {
      label: t("purchaseShoppingItem.fortryd"), run: () => undoShoppingItem(item)
    });
    return true;
  } catch (error) {
    if (quantity !== undefined) throw error;
    showToast(t("purchaseShoppingItem.varenKunneIkkeMarkeresSomKobtMessage", {message: error.message}), 'error');
    return false;
...
async function undoShoppingItem(item) {
  try {
    ...
    await Promise.all([loadShoppingList(), loadInventory()]); showToast(t("undoShoppingItem.nameErTilbagePaListen", {name: item.product.name}));
  } catch (error) { showToast(t("undoShoppingItem.kobetKunneIkkeFortrydesMessage", {message: error.message}), 'error'); }
}
...
async function searchShoppingCandidates(search) {
  ...
  } catch (error) { if (requestId === shoppingSearchRequestId) showToast(t("searchRecipeTemplates.sogningenMislykkedesMessage", {message: error.message}), 'error'); }
}
...
async function submitShoppingCandidate(quantity) {
  ...
  const payload = selectedShoppingCandidate.source === 'product'
    ...
    : (quantity == null ? {} : { quantity });
  try { await jsonRequest(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const name = selectedShoppingCandidate.name; closeShoppingAdd(); await Promise.all([loadShoppingList(), loadProducts()]); showToast(t("submitShoppingCandidate.nameErTilfojetTilIndkob", {name: name})); }
...
function setShoppingEditorBusy(busy) {
  ...
  document.querySelector('#purchase-edit-shopping').textContent = busy ? t("setShoppingEditorBusy.vent") : t("common.purchased");
}
...
async function saveShoppingItem(event) {
  ...
  try { await jsonRequest(`${SHOPPING_API}/items/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ quantity }) }); closeShoppingEditor(true); await loadShoppingList(); showToast(t("saveShoppingItem.varenErGemt")); }
...
async function deleteShoppingItem() {
  ...
  setShoppingEditorBusy(true);
  try { await jsonRequest(`${SHOPPING_API}/items/${id}`, { method: 'DELETE' }); closeShoppingEditor(true); await loadShoppingList(); showToast(t("deleteShoppingItem.varenErFjernetFraIndkobslisten")); }
  catch (error) { showMessage(document.querySelector('#edit-shopping-error'), error.message); }
  finally { setShoppingEditorBusy(false); }
}

const recipeUnitLabels = { GRAM:t("domain.g"), MILLILITER:t("domain.ml"), PIECE:t("domain.stk"), TEASPOON:t("domain.tsk"), TABLESPOON:t("domain.spsk"), DECILITER:t("domain.dl"), GRINDER_TURN:t("domain.omgange") };
...
function recipeCard(recipe) {
  ...
  const description = document.createElement('span'); description.textContent = recipe.description || t("recipeCard.lengthIngredienser", {length: recipe.ingredients.length});
...
async function loadRecipes() {
  ...
  catch (error) { showToast(t("loadRecipeTemplates.opskrifterneKunneIkkeHentesMessage", {message: error.message}), 'error'); }
...
function recipeUnitLabel(unit,value){return unit==='GRINDER_TURN'&&Number(value)===1?t("recipeUnitLabel.omgang"):recipeUnitLabels[unit];}
function renderProcessDetails(step){const rendered=step.renderedProcess||{},details=document.createElement('details');details.className='process-details';const summary=document.createElement('summary');summary.className='process-summary';const title=document.createElement('strong');title.textContent=rendered.processName||step.processName||t("openRecipeEditor.madlavningsproces");summary.append(title);if(rendered.durationSummary){const timing=document.createElement('span');timing.textContent=rendered.durationSummary;summary.append(timing);}if(rendered.inputSummary){const inputs=document.createElement('small');inputs.className='process-input-summary';inputs.textContent=rendered.inputSummary;summary.append(inputs);}const content=document.createElement('div');content.className='process-expanded';const instructions=document.createElement('ul');instructions.className='process-instructions';instructions.replaceChildren(...(rendered.instructions||[]).map(text=>{const item=document.createElement('li');item.textContent=text;return item;}));const completion=document.createElement('p');completion.className='process-completion';completion.textContent=t("renderProcessDetails.faerdigNarValue1", {value1: rendered.completionCriterion||t("renderProcessDetails.kontrollerResultatet")});const warnings=document.createElement('div');warnings.className='process-warnings';warnings.textContent=(rendered.warnings||[]).join(' · ');warnings.hidden=!warnings.textContent;content.append(instructions,completion,warnings);details.append(summary,content);return details;}

function renderCarbohydrates(value,host){if(!value){host.hidden=true;return;}host.hidden=false;const heading=document.createElement('strong');heading.textContent=t("renderNutritionAdmin.kulhydrat");const amount=document.createElement('div');amount.className='nutrition-amount';amount.textContent=t("renderCarbohydrates.value1GPrPortion", {value1: danishDecimal(value.perPortionGrams)});const total=document.createElement('small');total.textContent=t("renderCarbohydrates.value1GIAlt", {value1: danishDecimal(value.totalGrams)});host.replaceChildren(heading,amount,total);if(!value.complete){const warning=document.createElement('p');warning.className='nutrition-warning';warning.textContent=t("renderCarbohydrates.ukendtBidragFraUnknownIngredientCountValue2", {unknownIngredientCount: value.unknownIngredientCount, count: value.unknownIngredientCount});host.append(warning);}const known=(value.ingredients||[]).filter(i=>i.known);if(known.length){const details=document.createElement('details');const summary=document.createElement('summary');summary.textContent=t("renderCarbohydrates.visFordeling");const list=document.createElement('ul');known.forEach(i=>{const row=document.createElement('li');row.textContent=t("renderCarbohydrates.nameValue2G", {name: i.name, value2: danishDecimal(i.grams)});list.append(row);});details.append(summary,list);host.append(details);}}
function renderUnknownCarbohydrates(value,host){const unknown=(value?.ingredients||[]).filter(i=>!i.known);if(!unknown.length)return;const details=document.createElement('details');const summary=document.createElement('summary');summary.textContent=t("renderUnknownCarbohydrates.visUkendte");const heading=document.createElement('strong');heading.textContent=t("renderUnknownCarbohydrates.manglerKulhydratdata");const list=document.createElement('ul');unknown.forEach(i=>{const row=document.createElement('li');row.className='unknown-nutrition-row';const label=document.createElement('span');label.textContent=t("renderUnknownCarbohydrates.nameManglerData", {name: i.name});row.append(label);if(currentUser?.admin){const maintain=document.createElement('button');maintain.type='button';maintain.className='text-button';maintain.textContent=t("renderUnknownCarbohydrates.abnFodevaredata");maintain.onclick=()=>{pendingNutritionTemplateId=i.productTemplateId;document.querySelector('#nutrition-status').value='ALL';document.querySelector('#nutrition-dtu-status').value='ALL';document.querySelector('#nutrition-search').value='';document.querySelector('#recipe-detail-dialog').close();document.querySelector('#recipe-template-detail-dialog').close();showView('nutrition-admin');};row.append(maintain);}list.append(row);});details.append(summary,heading,list);host.append(details);}
...
async function openRecipe(id, initialPortions = 2) {
  ...
  } catch (error) { showToast(t("openRecipeTemplate.opskriftenKunneIkkeAbnesMessage", {message: error.message}), 'error'); }
}
...
function renderRecipeEditor() {
  ...
  document.querySelector('#recipe-editor-components').replaceChildren(...recipePreparedComponents.map((component,index)=>{const row=document.createElement('div');row.className='editor-item prepared-component-editor';const text=document.createElement('p');text.textContent=t("renderRecipeEditor.nameLengthIngredienser", {name: component.name, length: component.ingredients.length});row.append(text,editorActions(index,recipePreparedComponents,renderRecipeEditor));return row;}));
  document.querySelector('#recipe-editor-steps').replaceChildren(...recipeSteps.map((step, index) => {
    ...
    if (step.type === 'PROCESS') { const badge=document.createElement('small'); badge.className='process-badge'; badge.textContent=t("renderRecipeEditor.proces"); const title=document.createElement('strong'); title.textContent=step.processName || t("openRecipeEditor.madlavningsproces"); const edit=document.createElement('button'); edit.type='button'; edit.className='text-button'; edit.textContent=t("renderRecipeEditor.redigerProces"); edit.onclick=()=>openProcessPicker(index); body.append(badge,title,edit); }
    else { const input = document.createElement('textarea'); input.rows = 2; input.value = step.instruction; input.setAttribute('aria-label', t("renderRecipeEditor.trinValue1", {value1: index + 1})); input.addEventListener('input', () => { recipeSteps[index].instruction = input.value; }); body.append(input); }
...
function editorActions(index, collection, rerender) {
  ...
  [['↑',-1,t("editorActions.flytOp")],['↓',1,t("editorActions.flytNed")]].forEach(([label, delta, aria]) => { const button = document.createElement('button'); button.type='button'; button.textContent=label; button.setAttribute('aria-label', aria); button.disabled = index + delta < 0 || index + delta >= collection.length; button.onclick=() => { [collection[index], collection[index+delta]]=[collection[index+delta],collection[index]]; rerender(); }; actions.append(button); });
...
function openRecipeEditor(recipe = null) {
  editingRecipeId = recipe?.id || null; document.querySelector('#recipe-editor-title').textContent = recipe ? t("openRecipeEditor.redigerOpskrift") : t("openRecipeEditor.nyOpskrift");
  ...
  recipeSteps = (recipe?.steps || []).sort((a,b)=>a.sortOrder-b.sortOrder).map(s => ({ type:s.type || 'TEXT', instruction:s.instruction || '', cookingProcessId:s.cookingProcessId, processName:s.processName || t("openRecipeEditor.madlavningsproces"), parameterBindings:s.parameterBindings || [] }));
...
async function searchRecipeTemplates(search) {
  ...
  } catch (error) { if (requestId === recipeSearchRequestId) showToast(t("searchRecipeTemplates.sogningenMislykkedesMessage", {message: error.message}), 'error'); }
}
...
function addRecipeIngredient() {
  ...
  if (!selectedRecipeTemplate || !/^\d+(\.\d+)?$/.test(raw) || Number(raw) <= 0) { showMessage(document.querySelector('#recipe-error'),t("addRecipeIngredient.angivEnGyldigMaengdeStorreEndNul")); return; }
...
function openPreparedComponentPicker(){const host=document.querySelector('#prepared-component-ingredients');host.replaceChildren(...recipeIngredients.map(ingredient=>{const row=document.createElement('div');row.className='component-allocation-row';const label=document.createElement('label'),check=document.createElement('input');check.type='checkbox';check.value=ingredient.id;check.dataset.componentIngredient='true';label.append(check,document.createTextNode(` ${ingredient.template.name}`));const quantity=document.createElement('input');quantity.type='number';quantity.min='0.01';quantity.step='any';quantity.value=ingredient.quantity;quantity.dataset.componentQuantity='true';quantity.setAttribute('aria-label',t("openPreparedComponentPicker.maengdeName", {name: ingredient.template.name}));const unit=document.createElement('select');unit.dataset.componentUnit='true';Object.entries(recipeUnitLabels).forEach(([value,text])=>unit.append(new Option(text,value)));unit.value=ingredient.unit;row.append(label,quantity,unit);return row;}));document.querySelector('#prepared-component-picker').hidden=false;document.querySelector('#prepared-component-name').focus();}
...
async function openProcessPicker(stepIndex = null) {
  ...
  try {
    ...
    select.replaceChildren(new Option(t("openProcessPicker.vaelgProces"),''),...cookingProcesses.map(process=>new Option(process.name,process.id)));
    ...
    document.querySelector('#save-process-step').textContent=editingProcessStepIndex===null?t("openProcessPicker.tilfojProces"):t("openProcessPicker.gemProces");
  ...
  } catch(error) { picker.hidden=true; showMessage(document.querySelector('#recipe-error'),t("openProcessPicker.processerneKunneIkkeHentesMessage", {message: error.message})); }
}
...
function updateIngredientAllocation(wrapper,binding=null) {
  ...
  if(select?.value.startsWith('component:')){fields.hidden=true;context.textContent=t("updateIngredientAllocation.klargjortKomponentBrugesSomEnProcesinput");return;}
  ...
  const refresh=()=>{const allocated=allocatedBase/unitFactor(ingredient.unit),remaining=Math.max(0,totalBase-allocatedBase)/unitFactor(ingredient.unit);context.textContent=t("updateIngredientAllocation.opskriftIAltValue1Value2AlleredeFordelt", {value1: danishDecimal(ingredient.quantity), value2: recipeUnitLabels[ingredient.unit], value3: danishDecimal(allocated), value4: recipeUnitLabels[ingredient.unit], value5: danishDecimal(remaining), value6: recipeUnitLabels[ingredient.unit]});};
...
function ingredientParameterField(parameter,binding) {
  ...
  if(!recipeIngredients.length){const empty=document.createElement('p');empty.className='process-ingredient-empty';empty.textContent=t("addProcessStep.ingenIngredienserErTilfojetTilOpskriftenEndnu");wrapper.append(empty);return wrapper;}
  const selectLabel=document.createElement('label');selectLabel.textContent=t("ingredientParameterField.ingrediensEllerKlargjortKomponent");const select=document.createElement('select');select.dataset.ingredientSelect='true';select.append(new Option(t("ingredientParameterField.vaelgInput"),''));recipePreparedComponents.forEach(component=>select.append(new Option(component.name,`component:${component.id}`)));recipeIngredients.forEach(ingredient=>select.append(new Option(`${ingredient.template.name} · ${danishDecimal(ingredient.quantity)} ${recipeUnitLabels[ingredient.unit]}`,ingredient.id)));selectLabel.append(select);wrapper.append(selectLabel);
  const fields=document.createElement('div');fields.className='ingredient-allocation-fields';fields.hidden=true;const quantityLabel=document.createElement('label');quantityLabel.textContent=t("ingredientParameterField.brugIDenneProces");const allocation=document.createElement('div');allocation.className='quantity-unit-row';const quantity=document.createElement('input');quantity.type='number';quantity.min='0.01';quantity.step='any';quantity.inputMode='decimal';quantity.dataset.allocationQuantity='true';const unit=document.createElement('select');unit.dataset.allocationUnit='true';allocation.append(quantity,unit);quantityLabel.append(allocation);const context=document.createElement('small');context.className='ingredient-allocation-context';fields.append(quantityLabel,context);wrapper.append(fields);
...
function processParameterField(parameter) {
  ...
  const wrapper=document.createElement('label'); wrapper.className='process-parameter';wrapper.dataset.parameterKey=parameter.key;wrapper.dataset.parameterType=parameter.type;wrapper.dataset.required=String(parameter.required);wrapper.dataset.source=parameter.source||'INPUT';wrapper.textContent=`${parameter.label}${parameter.required?'':t("processParameterField.valgfri")}`;
  if(parameter.type==='DURATION'){const seconds=binding?.durationSeconds??parameter.defaultValue?.durationSeconds??0;const row=document.createElement('div');row.className='duration-input-row';const minutes=document.createElement('input');minutes.type='number';minutes.min='0';minutes.step='1';minutes.inputMode='numeric';minutes.dataset.durationMinutes='true';minutes.value=String(Math.floor(seconds/60));const minuteText=document.createElement('span');minuteText.textContent=t("processParameterField.minutter");const remainder=document.createElement('input');remainder.type='number';remainder.min='0';remainder.max='59';remainder.step='1';remainder.inputMode='numeric';remainder.dataset.durationSeconds='true';remainder.value=String(seconds%60);const secondText=document.createElement('span');secondText.textContent=t("processParameterField.sekunder");row.append(minutes,minuteText,remainder,secondText);wrapper.append(row);return wrapper;}
  let input;if(parameter.type==='HEAT_LEVEL'){input=document.createElement('select');['LOW','MEDIUM_LOW','MEDIUM','MEDIUM_HIGH','HIGH','MAX'].forEach(level=>input.append(new Option({LOW:t("processParameterField.lav"),MEDIUM_LOW:t("processParameterField.middelLav"),MEDIUM:t("processParameterField.middel"),MEDIUM_HIGH:t("processParameterField.middelHoj"),HIGH:t("processParameterField.hoj"),MAX:t("processParameterField.maksimal")}[level],level)));input.value=binding?.heatLevel||parameter.defaultValue?.heatLevel||'';}else{input=document.createElement('input');input.type=parameter.type==='TEXT'?'text':'number';if(parameter.type==='TEMPERATURE')input.min='0';else if(parameter.type==='QUANTITY')input.min='0.01';input.step=parameter.type==='NUMBER'||parameter.type==='QUANTITY'?'any':'1';if(parameter.type==='TEMPERATURE')input.value=binding?.temperatureCelsius??parameter.defaultValue?.temperatureCelsius??'';else if(parameter.type==='NUMBER')input.value=binding?.number??parameter.defaultValue?.number??'';else if(parameter.type==='QUANTITY')input.value=binding?.quantity??parameter.defaultValue?.quantity??'';else input.value=binding?.text??parameter.defaultValue?.text??'';}input.dataset.parameterInput='true';input.dataset.unit=parameter.unit||parameter.defaultValue?.unit||'';wrapper.append(input);if(parameter.type==='TEMPERATURE'){const help=document.createElement('small');help.textContent='°C';wrapper.append(help);}if(parameter.type==='QUANTITY'&&input.dataset.unit){const help=document.createElement('small');help.textContent=recipeUnitLabels[input.dataset.unit]||input.dataset.unit;wrapper.append(help);}return wrapper;
}
...
function selectCookingProcess() {selectedCookingProcess=cookingProcesses.find(process=>process.id===document.querySelector('#cooking-process-select').value)||null;document.querySelector('#cooking-process-description').textContent=selectedCookingProcess?.description||'';const host=document.querySelector('#cooking-process-parameters');host.replaceChildren();const parameters=(selectedCookingProcess?.parameters||[]).sort((a,b)=>a.sortOrder-b.sortOrder),normal=parameters.filter(p=>(p.source||'INPUT')==='INPUT'),advanced=parameters.filter(p=>(p.source||'INPUT')!=='INPUT'&&p.source!=='DEFAULT');host.append(...normal.map(processParameterField));if(advanced.length){const details=document.createElement('details');details.className='process-advanced';const summary=document.createElement('summary');summary.textContent=t("selectCookingProcess.avanceredeIndstillinger");details.append(summary,...advanced.map(processParameterField));host.append(details);}}

function addProcessStep() {
  if(!selectedCookingProcess){showMessage(document.querySelector('#recipe-error'),t("addProcessStep.vaelgEnMadlavningsproces"));return;}const bindings=[];
  for(const wrapper of document.querySelectorAll('#cooking-process-parameters .process-parameter')){const key=wrapper.dataset.parameterKey,type=wrapper.dataset.parameterType,required=wrapper.dataset.required==='true';const binding={parameterKey:key};
    if(type==='INGREDIENT_QUANTITY'){const select=wrapper.querySelector('[data-ingredient-select]');if(!select?.value){if(required){showMessage(document.querySelector('#recipe-error'),recipeIngredients.length?t("addProcessStep.vaelgEnIngrediensTilTextContent", {textContent: wrapper.querySelector('legend').textContent}):t("addProcessStep.ingenIngredienserErTilfojetTilOpskriftenEndnu"));select?.focus();return;}continue;}if(select.value.startsWith('component:')){binding.preparedComponentId=select.value.substring(10);bindings.push(binding);continue;}const ingredient=recipeIngredients.find(value=>value.id===select.value),quantity=wrapper.querySelector('[data-allocation-quantity]').value,unit=wrapper.querySelector('[data-allocation-unit]').value;if(!quantity||Number(quantity)<=0){showMessage(document.querySelector('#recipe-error'),t("addProcessStep.angivEnPositivMaengdeTilTextContent", {textContent: wrapper.querySelector('legend').textContent}));return;}const allocatedBase=allocatedForIngredient(ingredient.id),requestedBase=Number(quantity)*unitFactor(unit),totalBase=Number(ingredient.quantity)*unitFactor(ingredient.unit);if(allocatedBase+requestedBase>totalBase+1e-9){showMessage(document.querySelector('#recipe-error'),t("addProcessStep.nameErOverfordeltDerErKunValue2", {name: ingredient.template.name, value2: danishDecimal(Math.max(0,totalBase-allocatedBase)/unitFactor(ingredient.unit)), value3: recipeUnitLabels[ingredient.unit]}));return;}Object.assign(binding,{recipeIngredientId:ingredient.id,productTemplateId:ingredient.template.id,quantity,unit});}
    else if(type==='INGREDIENT_LIST'){for(const input of wrapper.querySelectorAll('[data-set-ingredient]:checked')){const ingredient=recipeIngredients.find(i=>i.id===input.dataset.setIngredient),member=input.closest('.ingredient-set-member'),quantity=member.querySelector('[data-set-quantity]').value,unit=member.querySelector('[data-set-unit]').value;if(!quantity||Number(quantity)<=0){showMessage(document.querySelector('#recipe-error'),t("addProcessStep.angivEnPositivMaengdeTilName", {name: ingredient.template.name}));return;}bindings.push({parameterKey:`${key}:${ingredient.id}`,recipeIngredientId:ingredient.id,productTemplateId:ingredient.template.id,quantity,unit});}continue;}
    else if(type==='DURATION'){const minutes=wrapper.querySelector('[data-duration-minutes]').value,seconds=wrapper.querySelector('[data-duration-seconds]').value;if(!/^\d+$/.test(minutes)||!/^\d+$/.test(seconds)||Number(seconds)>59){showMessage(document.querySelector('#recipe-error'),t("addProcessStep.angivHeleMinutterOg059Sekunder", {textContent: wrapper.firstChild.textContent}));return;}const total=Number(minutes)*60+Number(seconds);const original=currentProcessBinding(key)?.durationSeconds,standard=selectedCookingProcess.parameters.find(p=>p.key===key)?.defaultValue?.durationSeconds;if(total>0&&total!==standard)binding.durationSeconds=total;else if(original&&total===original&&original!==standard)binding.durationSeconds=total;else continue;}
    else{const input=wrapper.querySelector('[data-parameter-input]'),value=input.value;if(!value&&required){showMessage(document.querySelector('#recipe-error'),t("addProcessStep.udfyldTextContent", {textContent: wrapper.firstChild.textContent}));input.focus();return;}if(!value)continue;const parameter=selectedCookingProcess.parameters.find(p=>p.key===key),defaults=parameter?.defaultValue||{},defaultValue=type==='HEAT_LEVEL'?defaults.heatLevel:type==='TEMPERATURE'?defaults.temperatureCelsius:type==='NUMBER'?defaults.number:type==='QUANTITY'?defaults.quantity:defaults.text;if(wrapper.dataset.source!=='INPUT'&&!currentProcessBinding(key)&&String(value)===String(defaultValue??''))continue;if(type==='HEAT_LEVEL')binding.heatLevel=value;else if(type==='TEMPERATURE')binding.temperatureCelsius=Number(value);else if(type==='NUMBER')binding.number=value;else if(type==='QUANTITY'){binding.quantity=value;binding.unit=input.dataset.unit;}else binding.text=value.trim();}
...
async function saveRecipe(event) {
  ...
  if (!name || recipeSteps.some(step=>step.type !== 'PROCESS' && !step.instruction.trim())) { showMessage(document.querySelector('#recipe-error'),t("saveRecipe.navnOgAlleTeksttrinSkalVaereUdfyldt")); return; }
  ...
  try { const url=editingRecipeId ? `${RECIPE_API}/${editingRecipeId}` : RECIPE_API; await jsonRequest(url,{method:editingRecipeId?'PATCH':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); closeRecipeEditor(); closeRecipeDetail(); await loadRecipes(); showToast(t("saveCurrentMealPlan.nameErGemt", {name: name})); }
  catch(error){ showMessage(document.querySelector('#recipe-error'),error.message); }
}

async function deleteRecipe() { if (!currentRecipe) return; try { const name=currentRecipe.name; await jsonRequest(`${RECIPE_API}/${currentRecipe.id}`,{method:'DELETE'}); closeRecipeDetail(); await loadRecipes(); showToast(t("deleteMealPlan.nameErSlettet", {name: name})); } catch(error){ showToast(error.status===409?error.message:t("deleteRecipe.opskriftenKunneIkkeSlettesMessage", {message: error.message}),'error'); } }
...
function renderRecipePlanPreview(recipe, portions, container) {
  const label=document.createElement('p'); label.className='recipe-plan-preview-label'; label.textContent=t("renderRecipePlanPreview.skalBruge");
...
function renderRecipePlan() {
  document.querySelector('#recipe-plan-list').replaceChildren(...currentRecipes.map(recipe => {
    ...
    const controls=document.createElement('div'); controls.className='mini-portions'; const down=document.createElement('button'); down.type='button'; down.textContent='−'; const count=document.createElement('span'); count.textContent=t("renderRecipePlan.portionsPortioner", {portions: state.portions}); const up=document.createElement('button'); up.type='button'; up.textContent='+'; const preview=document.createElement('div'); preview.className='recipe-plan-preview';
...
function requirementResultRow(r){const row=document.createElement('div');row.className='requirement-row';const name=document.createElement('strong');name.textContent=r.productTemplate.name;row.append(name);
  ...
  if(r.trackingMode==='PRESENCE'){row.classList.add(r.satisfied?'satisfied':'missing');const status=document.createElement('span');status.className=`requirement-status${r.satisfied?'':' missing'}`;status.textContent=r.satisfied?t("requirementResultRow.paLager2"):t("requirementResultRow.mangler");row.append(status);if(r.plannedUsageCount>0){const usage=document.createElement('small');usage.textContent=t("requirementResultRow.brugesIPlannedUsageCountAndrePlanlagteValue2", {plannedUsageCount: r.plannedUsageCount, count: r.plannedUsageCount});row.append(usage);}return row;}
  const available=r.availableQuantity??0;const missing=r.missingQuantity??0;const reserved=Number(r.reservedQuantity??0);const needed=requirementAmount(r.displayRequiredQuantity??r.requiredQuantity,r.displayRequiredUnit??r.unit);if(reserved>0){row.append(requirementValue(t("requirementResultRow.duSkalBruge"),needed),requirementValue(t("requirementResultRow.paLager"),requirementAmount(r.physicalQuantity??0,r.unit)),requirementValue(t("requirementResultRow.reserveret"),requirementAmount(r.reservedQuantity,r.unit)),requirementValue(t("requirementResultRow.tilgaengelig"),requirementAmount(available,r.unit)));}else{row.append(requirementValue(t("requirementResultRow.duSkalBruge"),needed),requirementValue(t("requirementResultRow.duHar"),requirementAmount(r.physicalQuantity??0,r.unit)));}
  const status=document.createElement('span');if(r.satisfied){row.classList.add('satisfied');status.className='requirement-status';status.textContent=t("inventory.sufficient");}else{row.classList.add(Number(available)>0?'partial':'missing');status.className='requirement-status missing';status.textContent=t("requirementResultRow.manglerValue1", {value1: requirementAmount(missing,r.unit)});}row.append(status);return row;}
...
async function calculateRecipePlan(){const payload=selectedRecipePayload();if(!payload.recipes.length){showMessage(document.querySelector('#recipe-plan-error'),t("saveCurrentMealPlan.vaelgMindstEnOpskrift"));return;}const button=document.querySelector('#calculate-recipe-plan');button.disabled=true;try{renderRequirementResults(await jsonRequest(`${RECIPE_API}/calculate-requirements`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}));showMessage(document.querySelector('#recipe-plan-error'),'');}catch(error){showMessage(document.querySelector('#recipe-plan-error'),error.message);}finally{button.disabled=false;}}
async function addRecipeMissing(){const button=document.querySelector('#add-recipe-missing');button.disabled=true;try{await jsonRequest(`${RECIPE_API}/add-missing-to-shopping-list`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(selectedRecipePayload())});closeRecipePlan();showToast(t("addRecipeMissing.manglerneErTilfojetTilIndkob"));}catch(error){showMessage(document.querySelector('#recipe-plan-error'),error.message);}finally{button.disabled=false;}}
async function cookCurrentRecipe(){if(!currentRecipe)return;const button=document.querySelector('#cook-recipe');button.disabled=true;try{const result=await jsonRequest(`${RECIPE_API}/${currentRecipe.id}/cook`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({portions:recipePortions})});showToast(t("cookCurrentRecipe.nameErMarkeretSomLavet", {name: currentRecipe.name}));const summary=document.querySelector('#cook-recipe-summary');summary.textContent=result.warnings.length?result.warnings.join(' · '):t("cookCurrentRecipe.lageretErOpdateret");summary.classList.toggle('error',result.warnings.length>0);summary.hidden=false;await loadInventory();}catch(error){showToast(t("cookCurrentRecipe.opskriftenKunneIkkeMarkeresSomLavetMessage", {message: error.message}),'error');}finally{button.disabled=false;}}
...
async function openRecipeTemplate(id, initialPortions=2){try{recipeTemplatePortions=initialPortions;await loadRecipeTemplateDetail(id,recipeTemplatePortions);document.querySelector('#recipe-template-detail-title').textContent=currentRecipeTemplate.name;const description=document.querySelector('#recipe-template-detail-description');description.textContent=currentRecipeTemplate.description||'';description.hidden=!currentRecipeTemplate.description;showMessage(document.querySelector('#recipe-template-error'),'');document.querySelector('#recipe-template-detail-dialog').showModal();}catch(error){showToast(t("openRecipeTemplate.opskriftenKunneIkkeAbnesMessage", {message: error.message}),'error');}}
...
async function addRecipeTemplate(){if(!currentRecipeTemplate)return;const button=document.querySelector('#add-recipe-template');button.disabled=true;try{if(currentRecipeTemplate.added&&currentRecipeTemplate.userRecipeId){closeRecipeTemplate();showRecipeSection('recipes');await openRecipe(currentRecipeTemplate.userRecipeId);return;}const recipe=await jsonRequest(`${RECIPE_TEMPLATE_API}/${currentRecipeTemplate.id}/add-to-my-recipes`,{method:'POST'});currentRecipeTemplate.added=true;currentRecipeTemplate.userRecipeId=recipe.id;await Promise.all([loadRecipes(),loadRecipeTemplates(document.querySelector('#recipe-template-catalog-search').value)]);showToast(t("addRecipeTemplate.nameErFojetTilDineOpskrifter", {name: recipe.name}));renderRecipeTemplateDetail();}catch(error){showMessage(document.querySelector('#recipe-template-error'),error.message);}finally{button.disabled=false;}}
function mealPlanCard(plan){const button=document.createElement('button');button.type='button';button.className='recipe-card';const name=document.createElement('strong');name.textContent=plan.name;const meta=document.createElement('span');meta.textContent=plan.completed?t("renderMealPlan.lengthRetterFaerdig", {length: plan.recipes.length}):t("mealPlan.recipeCount", {count: plan.recipes.length});button.append(name,meta);button.onclick=()=>openMealPlan(plan.id);return button;}
async function loadMealPlans(){const loading=document.querySelector('#meal-plans-loading');loading.hidden=false;try{currentMealPlans=await jsonRequest(MEAL_PLAN_API);document.querySelector('#meal-plan-list').replaceChildren(...currentMealPlans.map(mealPlanCard));document.querySelector('#meal-plans-empty').hidden=currentMealPlans.length>0;}catch(error){showToast(t("loadMealPlans.madplanerneKunneIkkeHentesMessage", {message: error.message}),'error');}finally{loading.hidden=true;}}
async function saveCurrentMealPlan(event){event.preventDefault();const payload=selectedRecipePayload();const name=document.querySelector('#meal-plan-name').value.trim();if(!name||!payload.recipes.length){showMessage(document.querySelector('#recipe-plan-error'),!name?t("saveCurrentMealPlan.givMadplanenEtNavn"):t("saveCurrentMealPlan.vaelgMindstEnOpskrift"));return;}const button=event.currentTarget.querySelector('button[type="submit"]');button.disabled=true;try{await jsonRequest(MEAL_PLAN_API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,recipes:payload.recipes})});closeRecipePlan();showRecipeSection('plans');showToast(t("saveCurrentMealPlan.nameErGemt", {name: name}));}catch(error){showMessage(document.querySelector('#recipe-plan-error'),error.message);}finally{button.disabled=false;}}
function plannedStatus(status){return {PLANNED:t("plannedStatus.planlagt"),COOKED:'✓ Lavet',SKIPPED:t("plannedStatus.sprungetOver")}[status]||status;}
function plannedRecipeRow(item){const row=document.createElement('article');row.className='planned-recipe-row';const main=document.createElement('div');main.className='planned-recipe-main';const text=document.createElement('div');const name=document.createElement('strong');name.textContent=item.recipe?.name||item.recipeName;const portions=document.createElement('div');portions.className='product-meta';portions.textContent=t("common.portions", {count: item.portions});text.append(name,portions);const status=document.createElement('span');status.className='planned-recipe-status';status.textContent=plannedStatus(item.status);main.append(text,status);if(item.recipe){main.tabIndex=0;main.setAttribute('role','button');const open=()=>openRecipe(item.recipe.id,item.portions);main.onclick=open;main.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}}}row.append(main);
  const actions=document.createElement('div');actions.className='planned-recipe-actions';if(item.status==='PLANNED'){const down=document.createElement('button');down.type='button';down.textContent=t("plannedRecipeRow.portion2");down.disabled=item.portions===1;down.onclick=()=>changePlannedPortions(item,item.portions-1);const up=document.createElement('button');up.type='button';up.textContent=t("plannedRecipeRow.portion");up.onclick=()=>changePlannedPortions(item,item.portions+1);const cook=document.createElement('button');cook.type='button';cook.textContent=t("plannedRecipeRow.markerLavet");cook.onclick=()=>cookPlanned(item,cook);const skip=document.createElement('button');skip.type='button';skip.textContent=t("plannedRecipeRow.springOver");skip.onclick=()=>togglePlannedSkip(item);actions.append(down,up,cook,skip);}else if(item.status==='SKIPPED'&&item.recipe){const undo=document.createElement('button');undo.type='button';undo.textContent=t("plannedRecipeRow.planlaegIgen");undo.onclick=()=>togglePlannedSkip(item);actions.append(undo);}if(item.status!=='COOKED'){const remove=document.createElement('button');remove.type='button';remove.textContent=t("common.remove");remove.onclick=()=>removePlannedRecipe(item);actions.append(remove);}row.append(actions);return row;}
function renderMealPlan(plan){currentMealPlan=plan;document.querySelector('#meal-plan-detail-title').textContent=plan.name;document.querySelector('#meal-plan-detail-summary').textContent=plan.completed?t("renderMealPlan.lengthRetterFaerdig", {length: plan.recipes.length}):t("renderMealPlan.lengthPlanlagt", {length: plan.recipes.filter(r=>r.status==='PLANNED').length});document.querySelector('#meal-plan-recipes').replaceChildren(...[...plan.recipes].sort((a,b)=>a.sortOrder-b.sortOrder).map(plannedRecipeRow));document.querySelector('#meal-plan-results').hidden=true;document.querySelector('#delete-meal-plan-confirmation').hidden=true;}
async function openMealPlan(id){try{renderMealPlan(await jsonRequest(`${MEAL_PLAN_API}/${id}`));document.querySelector('#meal-plan-detail-dialog').showModal();}catch(error){showToast(t("openMealPlan.madplanenKunneIkkeAbnesMessage", {message: error.message}),'error');}}
...
async function removePlannedRecipe(item){try{const plan=await jsonRequest(`${MEAL_PLAN_API}/${currentMealPlan.id}/recipes/${item.id}`,{method:'DELETE'});renderMealPlan(plan);await loadMealPlans();showToast(t("removePlannedRecipe.value1ErFjernetFraMadplanen", {value1: item.recipe?.name||item.recipeName}));}catch(error){showMessage(document.querySelector('#meal-plan-error'),error.message);}}
...
async function addMealPlanMissing(){const button=document.querySelector('#meal-plan-add-missing');button.disabled=true;try{await jsonRequest(`${MEAL_PLAN_API}/${currentMealPlan.id}/add-missing-to-shopping-list`,{method:'POST'});showToast(t("addMealPlanMissing.manglerneErSikretPaIndkobslisten"));}catch(error){showMessage(document.querySelector('#meal-plan-error'),error.message);}finally{button.disabled=false;}}
async function deleteMealPlan(){try{const name=currentMealPlan.name;await jsonRequest(`${MEAL_PLAN_API}/${currentMealPlan.id}`,{method:'DELETE'});closeMealPlan();await loadMealPlans();showToast(t("deleteMealPlan.nameErSlettet", {name: name}));}catch(error){showMessage(document.querySelector('#meal-plan-error'),error.message);}}

const equipmentTypeLabels={STOVE:t("resetKitchenEquipmentForm.komfur"),OVEN:t("equipmentSummary.ovn"),POT:t("equipmentSummary.gryde"),PAN:t("equipmentSummary.stegepande"),AIR_FRYER:t("equipmentSummary.airfryer"),THERMOMETER:t("domain.stegetermometer"),MICROWAVE:t("equipmentSummary.mikroovn")};
const heatSourceLabels={INDUCTION:t("domain.induktion"),CERAMIC:t("domain.keramisk"),ELECTRIC:t("domain.elektrisk"),GAS:t("domain.gas"),OTHER:t("domain.andet")};
const ovenModeLabels={CONVENTIONAL:t("domain.almindeligOvn"),FAN:t("domain.varmluft"),GRILL:t("domain.grill")};
function liters(ml){return t("liters.value1Liter", {value1: formatQuantity(ml/1000)});}
function centimeters(mm){return t("centimeters.value1Cm", {value1: formatQuantity(mm/10)});}
function equipmentSummary(e){switch(e.equipmentType){case'STOVE':return `${heatSourceLabels[e.heatSource]||''}${e.minimumLevel!=null?t("equipmentSummary.trinMinimumLevelMaximumLevel", {minimumLevel: e.minimumLevel, maximumLevel: e.maximumLevel}):''}`;case'OVEN':return (e.ovenModes||[]).map(x=>ovenModeLabels[x]).join(' / ')||t("equipmentSummary.ovn");case'POT':return e.capacityMl?liters(e.capacityMl):t("equipmentSummary.gryde");case'PAN':return [e.diameterMm?centimeters(e.diameterMm):'',e.nonStick?'non-stick':''].filter(Boolean).join(' · ')||t("equipmentSummary.stegepande");case'AIR_FRYER':return e.capacityMl?liters(e.capacityMl):t("equipmentSummary.airfryer");case'THERMOMETER':return {INSTANT_READ:t("equipmentSummary.hurtigAflaesning"),PROBE:t("equipmentSummary.stegesonde"),OTHER:t("equipmentSummary.andenType")}[e.thermometerType]||t("equipmentSummary.termometer");case'MICROWAVE':return e.maxPowerWatts?t("equipmentSummary.maxPowerWattsWatt", {maxPowerWatts: e.maxPowerWatts}):t("equipmentSummary.mikroovn");}}
function equipmentCard(e){const button=document.createElement('button');button.type='button';button.className='equipment-card';const text=document.createElement('span');const name=document.createElement('strong');name.textContent=e.name;const meta=document.createElement('span');meta.className='product-meta';meta.textContent=equipmentSummary(e);text.append(name,meta);const badge=document.createElement('span');badge.className='preferred-badge';badge.textContent=e.preferred?t("equipmentCard.foretrukket"):'›';button.append(text,badge);button.onclick=()=>openKitchenEquipment(e);return button;}
async function loadKitchenEquipment(){const loading=document.querySelector('#kitchen-loading');loading.hidden=false;try{currentKitchenEquipment=await jsonRequest(KITCHEN_EQUIPMENT_API);document.querySelector('#kitchen-equipment-list').replaceChildren(...currentKitchenEquipment.map(equipmentCard));document.querySelector('#kitchen-empty').hidden=currentKitchenEquipment.length>0;}catch(error){showToast(t("loadKitchenEquipment.ditKokkenKunneIkkeHentesMessage", {message: error.message}),'error');}finally{loading.hidden=true;}}
...
function suggestedHeatMappings(){const source=document.querySelector('#stove-heat-source').value,minText=document.querySelector('#stove-minimum-level').value,maxText=document.querySelector('#stove-maximum-level').value,min=Number(minText),max=Number(maxText);const gas=source==='GAS'&&(!minText||!maxText);const values=gas?[t("suggestedHeatMappings.lavtBlus"),t("suggestedHeatMappings.middelLavtBlus"),t("suggestedHeatMappings.middelBlus"),t("suggestedHeatMappings.middelHojtBlus"),t("suggestedHeatMappings.hojtBlus"),t("suggestedHeatMappings.fuldtBlus")]:[.15,.30,.50,.70,.85,1].map(p=>String(Math.max(min,Math.min(max,Math.round(min+(max-min)*p)))));document.querySelectorAll('[data-heat]').forEach((input,index)=>input.value=values[index]);}
function resetKitchenEquipmentForm(){const form=document.querySelector('#kitchen-equipment-form');form.reset();editingKitchenEquipment=null;document.querySelector('#kitchen-equipment-id').value='';document.querySelector('#kitchen-equipment-type').disabled=false;document.querySelector('#kitchen-equipment-type').value='STOVE';document.querySelector('#kitchen-equipment-name').value=t("resetKitchenEquipmentForm.komfur");document.querySelector('#stove-minimum-level').value='1';document.querySelector('#stove-maximum-level').value='9';document.querySelector('#delete-kitchen-equipment').hidden=true;document.querySelector('#delete-kitchen-equipment-confirmation').hidden=true;showMessage(document.querySelector('#kitchen-equipment-error'),'');setEquipmentFields('STOVE');suggestedHeatMappings();}
function openKitchenEquipment(e=null){resetKitchenEquipmentForm();editingKitchenEquipment=e;if(e){document.querySelector('#kitchen-equipment-title').textContent=t("openKitchenEquipment.redigerUdstyr");document.querySelector('#kitchen-equipment-id').value=e.id;document.querySelector('#kitchen-equipment-type').value=e.equipmentType;document.querySelector('#kitchen-equipment-type').disabled=true;document.querySelector('#kitchen-equipment-name').value=e.name;document.querySelector('#kitchen-equipment-preferred').checked=e.preferred;setEquipmentFields(e.equipmentType);document.querySelector('#delete-kitchen-equipment').hidden=false;if(e.equipmentType==='STOVE'){document.querySelector('#stove-heat-source').value=e.heatSource;document.querySelector('#stove-minimum-level').value=e.minimumLevel??'';document.querySelector('#stove-maximum-level').value=e.maximumLevel??'';document.querySelectorAll('[data-heat]').forEach(input=>input.value=e.heatMappings?.[input.dataset.heat]||'');}if(e.equipmentType==='OVEN'){document.querySelectorAll('[data-oven-mode]').forEach(input=>input.checked=(e.ovenModes||[]).includes(input.dataset.ovenMode));document.querySelector('#oven-min-temperature').value=e.minimumTemperatureCelsius??'';document.querySelector('#oven-max-temperature').value=e.maximumTemperatureCelsius??'';}if(e.equipmentType==='POT')document.querySelector('#pot-capacity').value=e.capacityMl??'';if(e.equipmentType==='PAN'){document.querySelector('#pan-diameter').value=e.diameterMm??'';document.querySelector('#pan-non-stick').checked=Boolean(e.nonStick);}if(e.equipmentType==='AIR_FRYER'){document.querySelector('#air-fryer-capacity').value=e.capacityMl??'';document.querySelector('#air-fryer-min-temperature').value=e.minimumTemperatureCelsius??'';document.querySelector('#air-fryer-max-temperature').value=e.maximumTemperatureCelsius??'';}if(e.equipmentType==='THERMOMETER')document.querySelector('#thermometer-type').value=e.thermometerType||'OTHER';if(e.equipmentType==='MICROWAVE')document.querySelector('#microwave-power').value=e.maxPowerWatts??'';}else document.querySelector('#kitchen-equipment-title').textContent=t("openKitchenEquipment.tilfojUdstyr");document.querySelector('#kitchen-equipment-dialog').showModal();}
...
function nutritionStatusText(entry){return entry.status==='MISSING'?t("nutritionStatusText.manglerData"):entry.status==='KNOWN_ZERO'?t("nutritionStatusText.kendt0"):t("nutritionStatusText.harData");}
function dtuMatchText(entry){if(entry.dtuMapping?.status==='REQUIRES_REVIEW')return t("dtuMatchText.ndretDanishName", {danishName: entry.dtuMapping.food.danishName});if(entry.dtuMapping?.status==='APPROVED')return t("dtuMatchText.dTUDanishName", {danishName: entry.dtuMapping.food.danishName});const suggestion=entry.dtuSuggestion,candidate=suggestion?.candidates?.[0];if(suggestion?.resolution==='AUTO_EQUIVALENT_CARBOHYDRATE')return t("dtuMatchText.value1G100GAutoValgtFra", {value1: danishDecimal(suggestion.representative.carbohydrateGrams), equivalentCandidateCount: suggestion.equivalentCandidateCount});if(!candidate)return t("dtuMatchText.intetForslag");if(suggestion.classification==='REVIEW_REQUIRED'&&suggestion.candidates.length>1)return t("dtuMatchText.lengthMuligeMatches", {length: suggestion.candidates.length});return t("dtuMatchText.dTUDanishName", {danishName: candidate.food.danishName});}
...
function renderNutritionAdmin(){const queue=document.querySelector('#nutrition-dtu-status').value;const entries=visibleNutritionEntries();document.querySelector('#nutrition-review-tools').hidden=queue!=='READY';const rows=entries.map(entry=>{const row=document.createElement('article');row.className='nutrition-row';row.dataset.productTemplateId=entry.productTemplateId;const product=document.createElement('div');product.className='nutrition-product';const suggestion=entry.dtuSuggestion,candidate=suggestion?.candidates?.length===1?suggestion.candidates[0]:null;const selectable=nutritionQueue(entry)==='READY'&&candidate;const check=document.createElement('input');check.type='checkbox';check.setAttribute('aria-label',t("renderNutritionAdmin.vaelgDTUMatchForName", {name: entry.name}));check.hidden=!selectable;check.checked=selectedNutritionMappings.has(entry.productTemplateId);check.onchange=()=>{if(check.checked)selectedNutritionMappings.set(entry.productTemplateId,candidate.food);else selectedNutritionMappings.delete(entry.productTemplateId);renderNutritionBulkActions();};const name=document.createElement('div');name.className='nutrition-product-name';const strong=document.createElement('strong');strong.textContent=entry.name;const key=document.createElement('small');key.textContent=entry.key;name.append(strong,key);if(entry.aliases?.length){const aliases=document.createElement('small');aliases.className='nutrition-aliases';aliases.textContent=t("renderNutritionAdmin.aliasValue1", {value1: entry.aliases.join(', ')});name.append(aliases);}product.append(check,name);const status=document.createElement('div');status.className='nutrition-cell';status.dataset.label=t("renderNutritionAdmin.status");const badge=document.createElement('span');badge.className=`nutrition-status-badge ${entry.status==='MISSING'?'missing':''}`;badge.textContent=nutritionStatusText(entry);status.append(badge);const value=document.createElement('div');value.className='nutrition-cell';value.dataset.label=t("renderNutritionAdmin.kulhydrat");value.textContent=entry.nutrition?t("renderNutritionAdmin.value1GValue2Value3", {value1: danishDecimal(entry.nutrition.carbohydrateGrams), value2: danishDecimal(entry.nutrition.basisQuantity), value3: recipeUnitLabels[entry.nutrition.basisUnit]}):'—';const source=document.createElement('div');source.className='nutrition-cell';source.dataset.label=t("renderNutritionAdmin.kilde");source.textContent=entry.nutrition?.provider||entry.nutrition?.source||'—';const match=document.createElement('div');match.className='nutrition-cell';match.dataset.label=t("html.dTUMatch");match.textContent=dtuMatchText(entry);if(candidate){const detail=document.createElement('small');detail.textContent=t("renderNutritionAdmin.dTUFoodIdValue2G100GValue3", {foodId: candidate.food.foodId, value2: danishDecimal(candidate.food.carbohydrateGrams), value3: suggestion.classification==='EXACT'?t("renderNutritionAdmin.exact"):t("renderNutritionAdmin.high")});match.append(detail);}else if(suggestion?.reason){const reason=document.createElement('small');reason.textContent=suggestion.reason;match.append(reason);}if(entry.dtuMapping?.status==='REQUIRES_REVIEW'){const note=document.createElement('small');note.textContent=t("renderNutritionAdmin.godkendtValue1GNyValue2G", {value1: danishDecimal(entry.dtuMapping.approvedCarbohydrateGrams), value2: danishDecimal(entry.dtuMapping.food.carbohydrateGrams)});match.append(note);}const actions=document.createElement('div');actions.className='nutrition-row-actions';const edit=document.createElement('button');edit.type='button';edit.className='secondary-button';edit.textContent=t("renderNutritionAdmin.rediger");edit.onclick=()=>openNutritionEdit(entry);if(selectable){const approve=document.createElement('button');approve.type='button';approve.className='primary-button';approve.textContent=t("renderDtuResults.godkend");approve.onclick=()=>approveDtuFood(entry,candidate.food);const change=document.createElement('button');change.type='button';change.className='secondary-button';change.textContent=t("renderNutritionAdmin.skift");change.onclick=()=>openNutritionMatch(entry);actions.append(edit,approve,change);}else{const choose=document.createElement('button');choose.type='button';choose.className='secondary-button';choose.textContent=nutritionQueue(entry)==='NO_MATCH'?t("renderNutritionAdmin.sogIDTU"):t("renderNutritionAdmin.gennemga");choose.onclick=()=>openNutritionMatch(entry);actions.append(edit,choose);}row.append(product,status,value,source,match,actions);return row;});document.querySelector('#nutrition-admin-list').replaceChildren(...rows);renderNutritionCounts();renderNutritionBulkActions();updateSelectAllNutrition();if(pendingNutritionTemplateId){const target=document.querySelector(`[data-product-template-id="${pendingNutritionTemplateId}"]`);target?.scrollIntoView({block:'center'});target?.classList.add('highlight');pendingNutritionTemplateId=null;}}
...
function requestApproveSelectedDtu(){const count=selectedNutritionMappings.size;if(!count)return;document.querySelector('#nutrition-bulk-confirm-text').textContent=t("requestApproveSelectedDtu.duErVedAtGodkendeCountEksplicit", {count: count});document.querySelector('#nutrition-bulk-confirm-dialog').showModal();}
...
function renderDtuResults(foods){const host=document.querySelector('#nutrition-dtu-results');host.replaceChildren(...foods.map(food=>{const row=document.createElement('div');row.className='dtu-result';const text=document.createElement('span');const name=document.createElement('strong');name.textContent=food.danishName;const meta=document.createElement('small');meta.textContent=t("renderDtuResults.dTUFoodIdValue2G100GVersion", {foodId: food.foodId, value2: danishDecimal(food.carbohydrateGrams), datasetVersion: food.datasetVersion});text.append(name,meta);const approve=document.createElement('button');approve.type='button';approve.className='primary-button';approve.textContent=t("renderDtuResults.godkend");approve.onclick=()=>approveDtuFood(currentNutritionMatch,food);row.append(text,approve);return row;}));}
function openNutritionMatch(entry){currentNutritionMatch=entry;document.querySelector('#nutrition-match-title').textContent=t("openNutritionMatch.dTUMatchName", {name: entry.name});document.querySelector('#nutrition-dtu-search').value='';renderDtuResults((entry.dtuSuggestion?.candidates||[]).map(value=>value.food));document.querySelector('#nutrition-match-dialog').showModal();}
...
function openNutritionEdit(entry){const n=entry.nutrition||{};document.querySelector('#nutrition-template-id').value=entry.productTemplateId;document.querySelector('#nutrition-edit-title').textContent=t("openNutritionEdit.kulhydratdataName", {name: entry.name});document.querySelector('#nutrition-grams').value=n.carbohydrateGrams??'';document.querySelector('#nutrition-basis').value=n.basisQuantity??100;document.querySelector('#nutrition-basis-unit').value=n.basisUnit||'GRAM';document.querySelector('#nutrition-source').value=n.source||'';document.querySelector('#nutrition-provider').value=n.provider||'';document.querySelector('#nutrition-external-id').value=n.externalFoodId||'';document.querySelector('#nutrition-source-version').value=n.sourceVersion||'';document.querySelector('#nutrition-source-url').value=n.sourceUrl||'';document.querySelector('#nutrition-note').value=n.note||'';document.querySelector('#nutrition-edit-dialog').showModal();}
...
document.querySelector('#clear-purchased').addEventListener('click', async () => {
  try { await jsonRequest(`${SHOPPING_API}/purchased`, { method: 'DELETE' }); await loadShoppingList(); showToast(t("domain.kobteVarerErRyddet")); }
  catch (error) { showToast(t("domain.kobteVarerKunneIkkeRyddesMessage", {message: error.message}), 'error'); }
});
...
document.querySelector('#recipe-portions-down').addEventListener('click', async() => { if (recipePortions > 1) { recipePortions--; try{await reloadCurrentRecipeForPortions();}catch(error){showToast(t("domain.portionerneKunneIkkeOpdateresMessage", {message: error.message}),'error');} } });
document.querySelector('#recipe-portions-up').addEventListener('click', async() => { recipePortions++; try{await reloadCurrentRecipeForPortions();}catch(error){showToast(t("domain.portionerneKunneIkkeOpdateresMessage", {message: error.message}),'error');} });
...
document.querySelector('#recipe-template-portions-down').addEventListener('click',async()=>{if(recipeTemplatePortions>1&&currentRecipeTemplate){recipeTemplatePortions--;try{await loadRecipeTemplateDetail(currentRecipeTemplate.id,recipeTemplatePortions);}catch(error){showToast(t("domain.portionerneKunneIkkeOpdateresMessage", {message: error.message}),'error');}}});
document.querySelector('#recipe-template-portions-up').addEventListener('click',async()=>{if(currentRecipeTemplate){recipeTemplatePortions++;try{await loadRecipeTemplateDetail(currentRecipeTemplate.id,recipeTemplatePortions);}catch(error){showToast(t("domain.portionerneKunneIkkeOpdateresMessage", {message: error.message}),'error');}}});
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => navigator.serviceWorker.register('/service-worker.js')
    .catch(() => showMessage(authError, t("domain.appenVirkerMenOfflineUnderstottelseKunneIkke"))));
}
