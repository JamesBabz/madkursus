...
function requirementResultRow(r){const row=document.createElement('div');row.className='requirement-row';const name=document.createElement('strong');name.textContent=r.productTemplate.name;row.append(name);
  ...
  const available=r.availableQuantity??0;const missing=r.missingQuantity??0;const reserved=Number(r.reservedQuantity??0);if(reserved>0){row.append(requirementValue('Du skal bruge',requirementAmount(r.requiredQuantity,r.unit)),requirementValue('På lager',requirementAmount(r.physicalQuantity??0,r.unit)),requirementValue('Reserveret',requirementAmount(r.reservedQuantity,r.unit)),requirementValue('Tilgængelig',requirementAmount(available,r.unit)));}else{row.append(requirementValue('Du skal bruge',requirementAmount(r.requiredQuantity,r.unit)),requirementValue('Du har',requirementAmount(r.physicalQuantity??0,r.unit)));}
