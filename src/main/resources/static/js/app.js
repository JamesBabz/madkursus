...
function showView(view) {
  ...
  const moreNavActive = moreActive || productsActive || kitchenActive || nutritionActive;
...
function displayUnit(unit) {
  return { GRAM: 'g', MILLILITER: 'ml', PIECE: 'stk', TEASPOON: 'tsk', TABLESPOON: 'spsk', DECILITER: 'dl', GRINDER_TURN: 'omgange' }[unit] || unit;
}
...
function recipeUnitLabel(unit,value){return unit==='GRINDER_TURN'&&Number(value)===1?'omgang':recipeUnitLabels[unit];}
function renderProcessDetails(step){const rendered=step.renderedProcess||{},details=document.createElement('details');details.className='process-details';const summary=document.createElement('summary');summary.className='process-summary';const title=document.createElement('strong');title.textContent=rendered.processName||step.processName||'Madlavningsproces';summary.append(title);if(rendered.durationSummary){const timing=document.createElement('span');timing.textContent=rendered.durationSummary;summary.append(timing);}if(rendered.inputSummary){const inputs=document.createElement('small');inputs.className='process-input-summary';inputs.textContent=rendered.inputSummary;summary.append(inputs);}const content=document.createElement('div');content.className='process-expanded';const instructions=document.createElement('ul');instructions.className='process-instructions';instructions.replaceChildren(...(rendered.instructions||[]).map(text=>{const item=document.createElement('li');item.textContent=text;return item;}));const completion=document.createElement('p');completion.className='process-completion';completion.textContent=`Færdig når: ${rendered.completionCriterion||'Kontrollér resultatet.'}`;const warnings=document.createElement('div');warnings.className='process-warnings';warnings.textContent=(rendered.warnings||[]).join(' · ');warnings.hidden=!warnings.textContent;content.append(instructions,completion,warnings);details.append(summary,content);return details;}

function renderCarbohydrates(value,host){if(!value){host.hidden=true;return;}host.hidden=false;const heading=document.createElement('strong');heading.textContent='Kulhydrat';const amount=document.createElement('div');amount.className='nutrition-amount';amount.textContent=`${danishDecimal(value.perPortionGrams)} g pr. portion`;const total=document.createElement('small');total.textContent=`${danishDecimal(value.totalGrams)} g i alt`;host.replaceChildren(heading,amount,total);if(!value.complete){const warning=document.createElement('p');warning.className='nutrition-warning';warning.textContent=`+ ukendt bidrag fra ${value.unknownIngredientCount} ${value.unknownIngredientCount===1?'ingrediens':'ingredienser'}`;host.append(warning);}const known=(value.ingredients||[]).filter(i=>i.known);if(known.length){const details=document.createElement('details');const summary=document.createElement('summary');summary.textContent='Vis fordeling';const list=document.createElement('ul');known.forEach(i=>{const row=document.createElement('li');row.textContent=`${i.name}: ${danishDecimal(i.grams)} g`;list.append(row);});details.append(summary,list);host.append(details);}}
function renderUnknownCarbohydrates(value,host){const unknown=(value?.ingredients||[]).filter(i=>!i.known);if(!unknown.length)return;const details=document.createElement('details');const summary=document.createElement('summary');summary.textContent='Vis ukendte';const heading=document.createElement('strong');heading.textContent='Mangler kulhydratdata:';const list=document.createElement('ul');unknown.forEach(i=>{const row=document.createElement('li');row.className='unknown-nutrition-row';const label=document.createElement('span');label.textContent=`${i.name} · Mangler data`;row.append(label);if(currentUser?.admin){const maintain=document.createElement('button');maintain.type='button';maintain.className='text-button';maintain.textContent='Åbn Fødevaredata';maintain.onclick=()=>{pendingNutritionTemplateId=i.productTemplateId;document.querySelector('#nutrition-status').value='ALL';document.querySelector('#nutrition-dtu-status').value='ALL';document.querySelector('#nutrition-search').value='';document.querySelector('#recipe-detail-dialog').close();document.querySelector('#recipe-template-detail-dialog').close();showView('nutrition-admin');};row.append(maintain);}list.append(row);});details.append(summary,heading,list);host.append(details);}
...
function renderRecipeEditor() {
  ...
  document.querySelector('#recipe-editor-components').replaceChildren(...recipePreparedComponents.map((component,index)=>{const row=document.createElement('div');row.className='editor-item prepared-component-editor';const text=document.createElement('p');text.textContent=`${component.name} · ${component.ingredients.length} ingredienser`;row.append(text,editorActions(index,recipePreparedComponents,renderRecipeEditor));return row;}));
...
function openPreparedComponentPicker(){const host=document.querySelector('#prepared-component-ingredients');host.replaceChildren(...recipeIngredients.map(ingredient=>{const row=document.createElement('div');row.className='component-allocation-row';const label=document.createElement('label'),check=document.createElement('input');check.type='checkbox';check.value=ingredient.id;check.dataset.componentIngredient='true';label.append(check,document.createTextNode(` ${ingredient.template.name}`));const quantity=document.createElement('input');quantity.type='number';quantity.min='0.01';quantity.step='any';quantity.value=ingredient.quantity;quantity.dataset.componentQuantity='true';quantity.setAttribute('aria-label',`Mængde ${ingredient.template.name}`);const unit=document.createElement('select');unit.dataset.componentUnit='true';Object.entries(recipeUnitLabels).forEach(([value,text])=>unit.append(new Option(text,value)));unit.value=ingredient.unit;row.append(label,quantity,unit);return row;}));document.querySelector('#prepared-component-picker').hidden=false;document.querySelector('#prepared-component-name').focus();}
...
function updateIngredientAllocation(wrapper,binding=null) {
  ...
  if(select?.value.startsWith('component:')){fields.hidden=true;context.textContent='Klargjort komponent bruges som én procesinput.';return;}
...
function ingredientParameterField(parameter,binding) {
  ...
  const selectLabel=document.createElement('label');selectLabel.textContent='Ingrediens eller klargjort komponent';const select=document.createElement('select');select.dataset.ingredientSelect='true';select.append(new Option('Vælg input',''));recipePreparedComponents.forEach(component=>select.append(new Option(component.name,`component:${component.id}`)));recipeIngredients.forEach(ingredient=>select.append(new Option(`${ingredient.template.name} · ${danishDecimal(ingredient.quantity)} ${recipeUnitLabels[ingredient.unit]}`,ingredient.id)));selectLabel.append(select);wrapper.append(selectLabel);
...
function selectCookingProcess() {selectedCookingProcess=cookingProcesses.find(process=>process.id===document.querySelector('#cooking-process-select').value)||null;document.querySelector('#cooking-process-description').textContent=selectedCookingProcess?.description||'';const host=document.querySelector('#cooking-process-parameters');host.replaceChildren();const parameters=(selectedCookingProcess?.parameters||[]).sort((a,b)=>a.sortOrder-b.sortOrder),normal=parameters.filter(p=>(p.source||'INPUT')==='INPUT'),advanced=parameters.filter(p=>(p.source||'INPUT')!=='INPUT'&&p.source!=='DEFAULT');host.append(...normal.map(processParameterField));if(advanced.length){const details=document.createElement('details');details.className='process-advanced';const summary=document.createElement('summary');summary.textContent='Avancerede indstillinger';details.append(summary,...advanced.map(processParameterField));host.append(details);}}
...
function requirementResultRow(r){const row=document.createElement('div');row.className='requirement-row';const name=document.createElement('strong');name.textContent=r.productTemplate.name;row.append(name);
  ...
  const available=r.availableQuantity??0;const missing=r.missingQuantity??0;const reserved=Number(r.reservedQuantity??0);const needed=requirementAmount(r.displayRequiredQuantity??r.requiredQuantity,r.displayRequiredUnit??r.unit);if(reserved>0){row.append(requirementValue('Du skal bruge',needed),requirementValue('På lager',requirementAmount(r.physicalQuantity??0,r.unit)),requirementValue('Reserveret',requirementAmount(r.reservedQuantity,r.unit)),requirementValue('Tilgængelig',requirementAmount(available,r.unit)));}else{row.append(requirementValue('Du skal bruge',needed),requirementValue('Du har',requirementAmount(r.physicalQuantity??0,r.unit)));}
...
async function openRecipeTemplate(id){try{recipeTemplatePortions=2;await loadRecipeTemplateDetail(id,recipeTemplatePortions);document.querySelector('#recipe-template-detail-title').textContent=currentRecipeTemplate.name;const description=document.querySelector('#recipe-template-detail-description');description.textContent=currentRecipeTemplate.description||'';description.hidden=!currentRecipeTemplate.description;showMessage(document.querySelector('#recipe-template-error'),'');document.querySelector('#recipe-template-detail-dialog').showModal();}catch(error){showToast(`Opskriften kunne ikke åbnes. ${error.message}`,'error');}}
...
function nutritionStatusText(entry){return entry.status==='MISSING'?'Mangler data':entry.status==='KNOWN_ZERO'?'Kendt 0':'Har data';}
function dtuMatchText(entry){if(entry.dtuMapping?.status==='REQUIRES_REVIEW')return `Ændret: ${entry.dtuMapping.food.danishName}`;if(entry.dtuMapping?.status==='APPROVED')return `DTU: ${entry.dtuMapping.food.danishName}`;const suggestion=entry.dtuSuggestion,candidate=suggestion?.candidates?.[0];if(suggestion?.resolution==='AUTO_EQUIVALENT_CARBOHYDRATE')return `${danishDecimal(suggestion.representative.carbohydrateGrams)} g / 100 g · Auto-valgt fra ${suggestion.equivalentCandidateCount} tilsvarende DTU-poster`;if(!candidate)return 'Intet forslag';if(suggestion.classification==='REVIEW_REQUIRED'&&suggestion.candidates.length>1)return `${suggestion.candidates.length} mulige matches`;return `DTU: ${candidate.food.danishName}`;}
...
function renderNutritionAdmin(){const queue=document.querySelector('#nutrition-dtu-status').value;const entries=visibleNutritionEntries();document.querySelector('#nutrition-review-tools').hidden=queue!=='READY';const rows=entries.map(entry=>{const row=document.createElement('article');row.className='nutrition-row';row.dataset.productTemplateId=entry.productTemplateId;const product=document.createElement('div');product.className='nutrition-product';const suggestion=entry.dtuSuggestion,candidate=suggestion?.candidates?.length===1?suggestion.candidates[0]:null;const selectable=nutritionQueue(entry)==='READY'&&candidate;const check=document.createElement('input');check.type='checkbox';check.setAttribute('aria-label',`Vælg DTU-match for ${entry.name}`);check.hidden=!selectable;check.checked=selectedNutritionMappings.has(entry.productTemplateId);check.onchange=()=>{if(check.checked)selectedNutritionMappings.set(entry.productTemplateId,candidate.food);else selectedNutritionMappings.delete(entry.productTemplateId);renderNutritionBulkActions();};const name=document.createElement('div');name.className='nutrition-product-name';const strong=document.createElement('strong');strong.textContent=entry.name;const key=document.createElement('small');key.textContent=entry.key;name.append(strong,key);if(entry.aliases?.length){const aliases=document.createElement('small');aliases.className='nutrition-aliases';aliases.textContent=`Alias: ${entry.aliases.join(', ')}`;name.append(aliases);}product.append(check,name);const status=document.createElement('div');status.className='nutrition-cell';status.dataset.label='Status';const badge=document.createElement('span');badge.className=`nutrition-status-badge ${entry.status==='MISSING'?'missing':''}`;badge.textContent=nutritionStatusText(entry);status.append(badge);const value=document.createElement('div');value.className='nutrition-cell';value.dataset.label='Kulhydrat';value.textContent=entry.nutrition?`${danishDecimal(entry.nutrition.carbohydrateGrams)} g / ${danishDecimal(entry.nutrition.basisQuantity)} ${recipeUnitLabels[entry.nutrition.basisUnit]}`:'—';const source=document.createElement('div');source.className='nutrition-cell';source.dataset.label='Kilde';source.textContent=entry.nutrition?.provider||entry.nutrition?.source||'—';const match=document.createElement('div');match.className='nutrition-cell';match.dataset.label='DTU-match';match.textContent=dtuMatchText(entry);if(candidate){const detail=document.createElement('small');detail.textContent=`DTU ${candidate.food.foodId} · ${danishDecimal(candidate.food.carbohydrateGrams)} g/100 g · ${suggestion.classification==='EXACT'?'Exact':'High'}`;match.append(detail);}else if(suggestion?.reason){const reason=document.createElement('small');reason.textContent=suggestion.reason;match.append(reason);}if(entry.dtuMapping?.status==='REQUIRES_REVIEW'){const note=document.createElement('small');note.textContent=`Godkendt: ${danishDecimal(entry.dtuMapping.approvedCarbohydrateGrams)} g · ny: ${danishDecimal(entry.dtuMapping.food.carbohydrateGrams)} g`;match.append(note);}const actions=document.createElement('div');actions.className='nutrition-row-actions';const edit=document.createElement('button');edit.type='button';edit.className='secondary-button';edit.textContent='Redigér';edit.onclick=()=>openNutritionEdit(entry);if(selectable){const approve=document.createElement('button');approve.type='button';approve.className='primary-button';approve.textContent='Godkend';approve.onclick=()=>approveDtuFood(entry,candidate.food);const change=document.createElement('button');change.type='button';change.className='secondary-button';change.textContent='Skift';change.onclick=()=>openNutritionMatch(entry);actions.append(edit,approve,change);}else{const choose=document.createElement('button');choose.type='button';choose.className='secondary-button';choose.textContent=nutritionQueue(entry)==='NO_MATCH'?'Søg i DTU':'Gennemgå';choose.onclick=()=>openNutritionMatch(entry);actions.append(edit,choose);}row.append(product,status,value,source,match,actions);return row;});document.querySelector('#nutrition-admin-list').replaceChildren(...rows);renderNutritionCounts();renderNutritionBulkActions();updateSelectAllNutrition();if(pendingNutritionTemplateId){const target=document.querySelector(`[data-product-template-id="${pendingNutritionTemplateId}"]`);target?.scrollIntoView({block:'center'});target?.classList.add('highlight');pendingNutritionTemplateId=null;}}
...
function requestApproveSelectedDtu(){const count=selectedNutritionMappings.size;if(!count)return;document.querySelector('#nutrition-bulk-confirm-text').textContent=`Du er ved at godkende ${count} eksplicit valgte ${count===1?'match':'matches'}.`;document.querySelector('#nutrition-bulk-confirm-dialog').showModal();}
...
function renderDtuResults(foods){const host=document.querySelector('#nutrition-dtu-results');host.replaceChildren(...foods.map(food=>{const row=document.createElement('div');row.className='dtu-result';const text=document.createElement('span');const name=document.createElement('strong');name.textContent=food.danishName;const meta=document.createElement('small');meta.textContent=`DTU ${food.foodId} · ${danishDecimal(food.carbohydrateGrams)} g/100 g · version ${food.datasetVersion}`;text.append(name,meta);const approve=document.createElement('button');approve.type='button';approve.className='primary-button';approve.textContent='Godkend';approve.onclick=()=>approveDtuFood(currentNutritionMatch,food);row.append(text,approve);return row;}));}
function openNutritionMatch(entry){currentNutritionMatch=entry;document.querySelector('#nutrition-match-title').textContent=`DTU-match · ${entry.name}`;document.querySelector('#nutrition-dtu-search').value='';renderDtuResults((entry.dtuSuggestion?.candidates||[]).map(value=>value.food));document.querySelector('#nutrition-match-dialog').showModal();}
...
function openNutritionEdit(entry){const n=entry.nutrition||{};document.querySelector('#nutrition-template-id').value=entry.productTemplateId;document.querySelector('#nutrition-edit-title').textContent=`Kulhydratdata · ${entry.name}`;document.querySelector('#nutrition-grams').value=n.carbohydrateGrams??'';document.querySelector('#nutrition-basis').value=n.basisQuantity??100;document.querySelector('#nutrition-basis-unit').value=n.basisUnit||'GRAM';document.querySelector('#nutrition-source').value=n.source||'';document.querySelector('#nutrition-provider').value=n.provider||'';document.querySelector('#nutrition-external-id').value=n.externalFoodId||'';document.querySelector('#nutrition-source-version').value=n.sourceVersion||'';document.querySelector('#nutrition-source-url').value=n.sourceUrl||'';document.querySelector('#nutrition-note').value=n.note||'';document.querySelector('#nutrition-edit-dialog').showModal();}
...
document.querySelector('#recipe-portions-down').addEventListener('click', async() => { if (recipePortions > 1) { recipePortions--; try{await reloadCurrentRecipeForPortions();}catch(error){showToast(`Portionerne kunne ikke opdateres. ${error.message}`,'error');} } });
document.querySelector('#recipe-portions-up').addEventListener('click', async() => { recipePortions++; try{await reloadCurrentRecipeForPortions();}catch(error){showToast(`Portionerne kunne ikke opdateres. ${error.message}`,'error');} });
...
document.querySelector('#recipe-template-portions-down').addEventListener('click',async()=>{if(recipeTemplatePortions>1&&currentRecipeTemplate){recipeTemplatePortions--;try{await loadRecipeTemplateDetail(currentRecipeTemplate.id,recipeTemplatePortions);}catch(error){showToast(`Portionerne kunne ikke opdateres. ${error.message}`,'error');}}});
document.querySelector('#recipe-template-portions-up').addEventListener('click',async()=>{if(currentRecipeTemplate){recipeTemplatePortions++;try{await loadRecipeTemplateDetail(currentRecipeTemplate.id,recipeTemplatePortions);}catch(error){showToast(`Portionerne kunne ikke opdateres. ${error.message}`,'error');}}});
