...
public class RecipeAdapterImpl implements RecipePort {
 ...
 @Transactional public Recipe save(Recipe model){
  if(model.id()==null){RecipeEntity created=new RecipeEntity(null,model.userId(),model.name(),model.description(),model.createdAt(),model.updatedAt());addChildren(created,model);return map(repository.save(created));}
