...
public class RecipeService {
    ...
    private final RecipeInstructionRenderer instructionRenderer;
    private CarbohydrateCalculator carbohydrateCalculator=new CarbohydrateCalculator();
    @Autowired public RecipeService(RecipePort port,ProductTemplateService templates,CurrentUserProvider currentUser,MealPlanPort mealPlans,CookingProcessService processes,RecipeInstructionRenderer instructionRenderer,CarbohydrateCalculator carbohydrateCalculator){this.port=port;this.templates=templates;this.currentUser=currentUser;this.mealPlans=mealPlans;this.processes=processes;this.instructionRenderer=instructionRenderer;if(carbohydrateCalculator!=null)this.carbohydrateCalculator=carbohydrateCalculator;}
    public RecipeService(RecipePort port,ProductTemplateService templates,CurrentUserProvider currentUser,MealPlanPort mealPlans,CookingProcessService processes,RecipeInstructionRenderer instructionRenderer){this(port,templates,currentUser,mealPlans,processes,instructionRenderer,new CarbohydrateCalculator());}
    ...
    public Recipe createFromTemplate(RecipeTemplate template) {
        ...
        List<RecipeStep> steps=template.steps().stream().map(source->new RecipeStep(null,source.type(),source.instruction(),source.sortOrder(),source.cookingProcessId(),
                source.parameterBindings().stream().map(binding->copyBinding(binding,copiedIngredients,copiedComponents)).toList(),null,remap(source.structuredInstruction(),copiedIngredients,copiedComponents))).toList();
        validateAllocations(ingredients,steps,components);
        List<RecipePreparationStep> preparation=template.preparationSteps().stream().map(value->new RecipePreparationStep(UUID.randomUUID(),value.instruction(),value.sortOrder(),remap(value.structuredInstruction(),copiedIngredients,copiedComponents))).toList();
    ...
    @Transactional
    public void delete(UUID id){Recipe recipe=get(id);UUID userId=recipe.userId();if(mealPlans.existsPlannedByRecipeIdAndUserId(id,userId))throw new ConflictException(ACTIVE_PLAN_MESSAGE);mealPlans.detachHistoricalRecipeReferences(id,userId);try{port.deleteByIdAndUserId(id,userId);}catch(DataIntegrityViolationException exception){if(hasConstraint(exception,"planned_recipes_recipe_id_fkey"))throw new ConflictException(ACTIVE_PLAN_MESSAGE);throw exception;}}

    private boolean hasConstraint(Throwable failure,String expected){for(Throwable current=failure;current!=null;current=current.getCause())if(current instanceof org.hibernate.exception.ConstraintViolationException violation&&expected.equals(violation.getConstraintName()))return true;return false;}
    ...
    private RecipeStructuredInstruction remap(RecipeStructuredInstruction value,Map<UUID,RecipeIngredient> ingredients,Map<UUID,PreparedComponent> components){if(value==null)return null;return new RecipeStructuredInstruction(value.parts().stream().map(p->{RecipeIngredient ingredient=p.recipeIngredientId()==null?null:ingredients.get(p.recipeIngredientId());PreparedComponent component=p.preparedComponentId()==null?null:components.get(p.preparedComponentId());return new RecipeInstructionPart(p.text(),ingredient==null?null:ingredient.id(),p.quantity(),p.unit(),component==null?null:component.id(),p.scaledNumber());}).toList());}
    ...
    private Recipe render(Recipe recipe,BigDecimal portions){List<RecipeStep> rendered=recipe.steps().stream().map(s->s.type()==RecipeStepType.PROCESS?new RecipeStep(s.id(),s.type(),null,s.sortOrder(),s.cookingProcessId(),s.parameterBindings(),processes.render(s.cookingProcessId(),scaledBindings(s.parameterBindings(),portions)),s.structuredInstruction()):s.structuredInstruction()==null?s:new RecipeStep(s.id(),s.type(),instructionRenderer.render(s.structuredInstruction(),recipe.ingredients(),recipe.preparedComponents(),portions),s.sortOrder(),null,List.of(),null,s.structuredInstruction())).toList();List<String> equipment=processes.equipmentOverview(recipe.steps().stream().filter(s->s.type()==RecipeStepType.PROCESS).map(RecipeStep::cookingProcessId).toList(),recipe.equipmentRequirements());List<RecipePreparationStep> explicit=recipe.preparationSteps().stream().map(p->p.structuredInstruction()==null?p:new RecipePreparationStep(p.id(),instructionRenderer.render(p.structuredInstruction(),recipe.ingredients(),recipe.preparedComponents(),portions),p.sortOrder(),p.structuredInstruction())).toList();List<RecipePreparationStep> preparation=aggregatePreparation(explicit,rendered);List<PreparedComponent> components=recipe.preparedComponents().stream().map(c->scaleComponent(c,portions)).toList();List<RecipeIngredient> scaledIngredients=recipe.ingredients().stream().map(i->new RecipeIngredient(i.id(),i.productTemplate(),i.quantity().multiply(portions),i.unit(),i.preparation(),i.sortOrder())).toList();return new Recipe(recipe.id(),recipe.userId(),recipe.sourceTemplateId(),recipe.name(),recipe.description(),recipe.createdAt(),recipe.updatedAt(),scaledIngredients,rendered,preparation,recipe.equipmentRequirements(),equipment,components,carbohydrateCalculator.calculateRecipe(recipe.ingredients(),portions));}
