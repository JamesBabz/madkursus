...
public class CookingProcessService {
    ...
    private void validateValue(CookingProcessParameter p,CookingProcessBinding b){if(b.preparedComponent()!=null){if(p.type()!=CookingProcessParameterType.INGREDIENT_QUANTITY&&p.type()!=CookingProcessParameterType.INGREDIENT_LIST)throw new InvalidInputException("Prepared component requires an ingredient process input");return;}CookingProcessValue v=b.value();if(v==null)throw new InvalidInputException("Parameter value is required: "+p.label());boolean valid=switch(p.type()){case INGREDIENT_QUANTITY->b.recipeIngredientId()!=null&&b.productTemplate()!=null&&v.quantity()!=null&&v.quantity().signum()>0&&v.unit()!=null;case INGREDIENT_LIST->false;case QUANTITY->v.quantity()!=null&&v.quantity().signum()>0&&(v.unit()!=null||p.unit()!=null);case DURATION->v.durationSeconds()!=null&&v.durationSeconds()>0;case TEMPERATURE->v.temperatureCelsius()!=null;case HEAT_LEVEL->v.heatLevel()!=null;case NUMBER->v.number()!=null;case TEXT->v.text()!=null&&!v.text().isBlank();};if(!valid)throw new InvalidInputException("Invalid value for process parameter: "+p.label());}
