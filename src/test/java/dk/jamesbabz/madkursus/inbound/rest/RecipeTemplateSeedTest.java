...
class RecipeTemplateSeedTest {
    ...
    void canonicalSourceContainsUniqueBaselineAndExternallyAuthoredTemplatesThatAllValidate() throws Exception {
        ...
        for(JsonNode recipe:recipes){
            ...
            if("KOEDBOLLER_I_TOMATSOVS_MED_PASTA".equals(recipe.path("key").asText())){assertThat(ingredients).containsKey("RAPSOLIE").doesNotContainKey("NEUTRAL_OLIE");Path draft=temp.resolve("imported.json");mapper.writeValue(draft.toFile(),recipe);assertThat(validator.validate(draft).key()).isEqualTo("KOEDBOLLER_I_TOMATSOVS_MED_PASTA");}
        }
