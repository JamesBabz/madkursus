...
class CookingProcessSeedTest {
    ...
    @Test void curatedLibraryIsCompleteAndInternallyValid() throws Exception {
        ...
        assertThat(processes.size()).isBetween(8,20);
        ...
        for(JsonNode process:processes) {
            ...
            for(JsonNode parameter:process.get("parameters")){assertThat(parameters.add(parameter.get("key").asText())).isTrue();assertThat(parameter.get("type").asText()).isIn("INGREDIENT_QUANTITY","INGREDIENT_LIST","QUANTITY","DURATION","TEMPERATURE","HEAT_LEVEL","NUMBER","TEXT");order++;}
        ...
        assertThat(names).contains("Kog kartofler","Rør fars","Steg kyllingebryst","Steg kødboller");
    }
    ...
    private JsonNode read()throws Exception{try(var stream=getClass().getClassLoader().getResourceAsStream("seed/cooking-processes.json")){assertThat(stream).isNotNull();return new ObjectMapper().readTree(stream);}}
}
